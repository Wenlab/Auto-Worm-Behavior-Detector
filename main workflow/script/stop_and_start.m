...
if path ~= 0
    ...
    if tf==1
        for i = indx
            ...
            list_csv = get_all_files_of_a_certain_type_in_a_rootpath(path,'machine_label*.csv');
            
            % if contains at least 1 suitable csv
            if numel(list_csv) ~= 0
                for k = 1:size(list_csv,2)
                    full_path_to_csv = list_csv{k};
